...
#핵심 Neural Network
W1 = tf.Variable(tf.truncated_normal(shape=[3, 3, 3, 32], stddev=0.01))
B1 = tf.Variable(tf.constant(0.1, shape=[32]))
H1 = tf.nn.relu(tf.nn.conv2d(X, W1, strides=[1, 1, 1, 1], padding='SAME') + B1)
W1_2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 32, 64], stddev=0.01))
...
W2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 64, 128], stddev=0.01))
B2 = tf.Variable(tf.constant(0.1, shape=[128]))
H2 = tf.nn.relu(tf.nn.conv2d(P1, W2, strides=[1, 1, 1, 1], padding='SAME') + B2)
W2_2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 128], stddev=0.01))
B2_2 = tf.Variable(tf.constant(0.1, shape=[128]))
H2_2 = tf.nn.relu(tf.nn.conv2d(H2, W2_2, strides=[1, 1, 1, 1], padding='SAME') + B2_2)
W2_3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 256], stddev=0.01))
...
W3_2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=0.01))
B3_2 = tf.Variable(tf.constant(0.1, shape=[512]))
H3_2 = tf.nn.relu(tf.nn.conv2d(H3, W3_2, strides=[1, 1, 1, 1], padding='SAME') + B3_2)
W3_3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 1024], stddev=0.01))
B3_3 = tf.Variable(tf.constant(0.1, shape=[1024]))
H3_3 = tf.nn.relu(tf.nn.conv2d(H3_2, W3_3, strides=[1, 1, 1, 1], padding='SAME') + B3_3)
P3 = tf.nn.max_pool(H3_3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

W4 = tf.Variable(tf.truncated_normal(shape=[3, 3, 1024, 2048], stddev=0.01))
B4 = tf.Variable(tf.constant(0.1, shape=[2048]))
H4 = tf.nn.relu(tf.nn.conv2d(P3, W4, strides=[1, 1, 1, 1], padding='SAME') + B4)

W5 = tf.Variable(tf.truncated_normal(shape=[3, 3, 2048, 2048], stddev=0.01))
B5 = tf.Variable(tf.constant(0.1, shape=[2048]))
H5 = tf.nn.relu(tf.nn.conv2d(H4, W5, strides=[1, 1, 1, 1], padding='SAME') + B5)

W_fc1 = tf.Variable(tf.truncated_normal(shape=[4 * 4 * 2048, 4096], stddev=0.01))
B_fc1 = tf.Variable(tf.constant(0.1, shape=[4096]))

H4_flat = tf.reshape(H4, [-1, 4*4*2048])
H_fc1 = tf.nn.relu(tf.matmul(H4_flat, W_fc1) + B_fc1)

H_fc1_drop = tf.nn.dropout(H_fc1, dropout_prob) 

W_fc2 = tf.Variable(tf.truncated_normal(shape=[4096, 10], stddev=0.01))
