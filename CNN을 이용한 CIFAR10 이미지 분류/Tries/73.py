...
W2_2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 256, 512], stddev=0.01))
B2_2 = tf.Variable(tf.constant(0.1, shape=[512]))
H2_2 = tf.nn.relu(tf.nn.conv2d(H2, W2_2, strides=[1, 1, 1, 1], padding='SAME') + B2_2)
W2_3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=0.01))
B2_3 = tf.Variable(tf.constant(0.1, shape=[512]))
...
W_fc1 = tf.Variable(tf.truncated_normal(shape=[8 * 8 * 512, 2048], stddev=0.01))
B_fc1 = tf.Variable(tf.constant(0.1, shape=[2048]))

Flat = tf.reshape(P2, [-1, 8*8*512])
H_fc1 = tf.nn.relu(tf.matmul(Flat, W_fc1) + B_fc1)

H_fc1_drop = tf.nn.dropout(H_fc1, dropout_prob)

W_fc1_2 = tf.Variable(tf.truncated_normal(shape=[2048, 512], stddev=0.01))
